...
{
	OLED_ShowNum(hang,add,temp/100%10+30,1,16);
	OLED_ShowNum(hang+8,add,temp/10%10+30,1,16);
	Oled_ShowString(hang+16,add,(uint8_t *)".");					//显示 .
	OLED_ShowNum(hang+24,add,temp/1%10+30,1,16);
...
{
	OLED_ShowNum(hang+8,add,humi/10%10+30,1,16);
	OLED_ShowNum(hang+16,add,humi/1%10+30,1,16);
	Oled_ShowString(hang+24,add,(uint8_t *)"%");					//显示 .
}
...
void OLED_Show_Time(unsigned char *TIME)
{
	...
	OLED_ShowNum(0,0,20,2,16);				//显示年
	OLED_ShowNum(16,0,nian,2,16);				//显示年
	Oled_ShowString(32,0,(unsigned char *)"-");					//显示 -
	OLED_ShowNum(40,0,yue,2,16);				//显示月
	Oled_ShowString(56,0,(unsigned char *)"-");					//显示 -
	OLED_ShowNum(64,0,ri,2,16);				//显示日

	OLED_ShowNum(0,2,shi,2,16);				//显示时
	Oled_ShowString(16,2,(unsigned char *)":");					//显示 :
	OLED_ShowNum(24,2,fen,2,16);				//显示分
	Oled_ShowString(40,2,(unsigned char *)":");					//显示 :
	OLED_ShowNum(48,2,miao,2,16);			//显示秒
